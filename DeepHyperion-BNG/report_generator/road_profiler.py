...
def get_poligon_from_geometry(geometry, side=None):
    if side is None:
    ...
    if side is "left":
        left_edge_x = np.array([e['left'][0] for e in geometry])
        left_edge_y = np.array([e['left'][1] for e in geometry])
        right_edge_x = np.array([e['middle'][0] for e in geometry])
        right_edge_y = np.array([e['middle'][1] for e in geometry])
        ...
        return Polygon(list(l_edge) + list(r_edge))
    if side is "right":
        left_edge_x = np.array([e['middle'][0] for e in geometry])
        left_edge_y = np.array([e['middle'][1] for e in geometry])
        right_edge_x = np.array([e['right'][0] for e in geometry])
        right_edge_y = np.array([e['right'][1] for e in geometry])
        ...
        return Polygon(list(l_edge) + list(r_edge))
    raise Exception("Wrong side provided: " + str(side))
...
class RoadSegment:
    ...
    def __init__(self, length, max_speed, curvature, geometry, direction):
        ...
        # TODO Are those ever user?
        self.left_polygon = get_poligon_from_geometry(geometry, side="left")
        self.right_polygon = get_poligon_from_geometry(geometry, side="right")
...
class RoadProfiler:
    ...
    def compute_sectors_by_driving_distance(self, number_of_sectors):
        ...
        sector_target_travel_distance = round(self.road.total_length / number_of_sectors)
        ...
        # DEBUG Visualize road segments and sectors
        # colors = ['blue', 'red']
        # s_colors = ['green', 'black']
        # plt.figure()
        # plt.gca().set_aspect('equal', 'box')
        #
        # for idx, segment in enumerate(self.road.road_segments):
        #     plt.plot(*segment.polygon.exterior.xy, color=colors[idx % 2])
        #
        # for idx, sector in enumerate(sectors):
        #     sector_polygon = cascaded_union([segment.polygon for segment in sector])
        #     plt.plot(*sector_polygon.exterior.xy, color=s_colors[idx % 2], linewidth=2)
        #
